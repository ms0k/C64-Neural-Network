...
total_classes=10
ds_train=torchvision.datasets.MNIST('MNIST', train=True, download=True, transform=image_transform)
ds_val=torchvision.datasets.MNIST('MNIST', train=False, download=True, transform=image_transform)
dl_train = torch.utils.data.DataLoader(ds_train, batch_size=4096, shuffle=True, generator=torch.Generator(device=train_device))
dl_val = torch.utils.data.DataLoader(ds_val, batch_size=2048, shuffle=True, generator=torch.Generator(device=train_device))
...
def train(model, epochs, device, save_every_epochs=0):
	...
	for epoch in tqdm.tqdm(range(epochs)):
		...
		for images, labels in dl_train:
			...
			outputs=model(img)
			loss=loss_fn(outputs, labels)+model.add_loss*.0035 #Adding the range outliers loss to force all numbers back into the (-1, 1) range
			with torch.no_grad():
				train_loss+=loss.item()/labels.shape[0]
				train_loss_add+=model.add_loss.item()
				train_acc+=(torch.sum(torch.argmax(outputs, dim=1)==labels)/labels.shape[0]).item()
			loss.backward()
			optimizer.step()
		...
		with torch.no_grad():
			...
			for images, labels in dl_val:
				...
				val_acc+=(torch.sum(torch.argmax(outputs, dim=1)==labels)/labels.shape[0]).item()
		val_acc/=len(dl_val)
		val_loss/=len(dl_val)
		...
		if save_every_epochs:
			if (epoch%save_every_epochs)==0 and epoch:
				trainstate={"optimizer": optimizer.state_dict(), "model": model, "epochs":epoch} #"scheduler": scheduler.state_dict(), 
...
torch.onnx.export(model.to(torch.device("cpu")), example_input.to(torch.device("cpu")), "mnist.onnx", input_names=["Image_Input"], output_names=["Numeral_OneHot"], opset_version=12)
